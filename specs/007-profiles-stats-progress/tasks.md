...
**Purpose**: Capture remaining gaps found by convergence review so the Phase 07 implementation can fully satisfy the spec, plan, constitution gates, and task list.

- [X] T095 CRITICAL: Add missing backend profile handler and route tests for current-profile, public stats, game history, CSRF rejection, guest denial, malformed bodies, invalid IDs, missing/disabled users, pagination, stable error envelopes, and profile update rate limiting in `backend/internal/profiles/handler_test.go` and `backend/internal/app/routes_test.go` per Constitution II / T024 / T041 / T054 / T068 / T069 (missing)
- [X] T096 CRITICAL: Make required release gates pass or document an accepted plan exception, including backend `gofmt`/`go test ./...`, targeted backend tests, OpenAPI validation, frontend lint/typecheck/build/tests, and migration readiness evidence in `specs/007-profiles-stats-progress/quickstart.md` per Constitution II / T081 / T082 / T083 / T084 / T085 / T086 / T087 / T088 (partial)
- [X] T097 Complete profile update validation for display-name control characters, country code normalization or allowlist behavior, timezone identifiers, safe avatar references, and preference key/value allowlists with matching service and handler tests in `backend/internal/profiles/service.go`, `backend/internal/profiles/service_test.go`, and `backend/internal/profiles/handler_test.go` per FR-004 / FR-006 / T011 / T012 / T067 / T068 (partial)
- [X] T098 Expand frontend profile tests to cover form validation, save success, save failure, rate-limited and unauthorized outcomes, pending/disabled behavior, private-field absence, Arabic RTL states, and localized public stats/history boundaries in `client/features/profile/*.test.tsx` per FR-007 / FR-016 / FR-017 / SC-007 / SC-008 / T026 / T042 / T070 (partial)
- [X] T099 Add privacy-safe structured logs for profile reads, profile updates, validation failures, public stats reads, history reads, denied guest access, and rate-limit rejections, then verify logged fields exclude raw emails, tokens, private preferences, precise private location data, hidden answers, coordinates, and provider metadata in `backend/internal/profiles/service.go` and `backend/internal/profiles/handler.go` per Constitution V / T030 / T058 / T073 / T094 (missing)
- [X] T100 Wire profile update rate-limit rejection observability by integrating the profile rate-limit path with `profiles.Metrics.RecordRateLimited` or replacing the unused metric with a tested route-level metric/log hook in `backend/internal/profiles/metrics.go` and `backend/internal/app/routes.go` per T072 / T074 (partial)
- [X] T101 Delegate, remove, or clearly retire the stale `backend/internal/users` stats/history route behavior so `/users/{userId}/stats` and `/users/{userId}/games` have a single Phase 07 privacy-safe owner, with tests proving the old contract cannot be mounted accidentally, per T046 / T060 (partial)
- [X] T102 Record real registered-browser and performance evidence for English profile update success, Arabic RTL profile/stats/history, keyboard focus, accessible names, disabled states, live status messages, non-color-only indicators, and profile/stats/history latency budgets in `specs/007-profiles-stats-progress/quickstart.md` per SC-001 / SC-002 / SC-007 / SC-008 / T089 / T090 / T091 / T092 / T093 (partial)
